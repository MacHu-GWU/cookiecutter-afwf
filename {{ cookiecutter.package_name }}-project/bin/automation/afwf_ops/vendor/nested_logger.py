...
def format_ruler(
    ...
    if _pipes is None:
        ...
        if len(_pipes) != nest:
            raise ValueError
    nesting = "".join(_pipes)
    s = f"{nesting}{corner}{left_pad}{msg:{char}{align.value}{length{% raw %}}}{% endraw %}{right_pad}{corner}"
    return s
...
class NestedLogger:
    ...
    def start_and_end(
        ...
        return self.pretty_log(
            ...
            error_msg=f"⏰ {error_emoji}Error {msg!r}, elapsed = {% raw %}{{{% endraw %}elapsed:.2f{% raw %}}}{% endraw %} sec",
            end_msg=f"⏰ {end_emoji}End {msg!r}, elapsed = {% raw %}{{{% endraw %}elapsed:.2f{% raw %}}}{% endraw %} sec",
            pipe=pipe,
        )
